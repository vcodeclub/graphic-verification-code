...
class GraphicVerificationCode(object):
    ...
    def generate(self, size=(120, 30), chars=None, mode='RGB', bg_color=(255, 255, 255), fg_color=(0, 0, 255), font_size=18, font_file=None, length=4, draw_lines=True, line_range=(1, 2), draw_points=True, point_chance=2, random_color=True):

        """
        @:param size: 图片的大小，格式（宽，高），默认为 (120, 30)
        @:param chars: 允许的字符集合，格式字符串
        @:param mode: 图片模式，默认为 RGB
        @:param bg_color: 背景色，默认为 白色
        @:param fg_color: 前景色，验证码字符颜色，默认为 蓝色 #0000FF，只有 random_color 为 False 时有效
        @:param font_size: 验证码字体大小
        @:param font_file: 验证码字体，默认为 None
        @:param length: 验证码字符个数，默认为 4
        @:param draw_lines: 是否划干扰线，默认为 是
        @:param line_range: 干扰线的条数范围，格式元组，默认为 (1, 2)，只有 draw_lines 为 True 时有效
        @:param draw_points: 是否画干扰点，默认为 是
        @:param point_chance: 干扰点出现的概率，大小范围 [0, 100]，只有 draw_points 为 True 时有效
        @:param random_color: 是否随机生成填充色，默认为 是
        ...
        def create_line():
            """ 绘制干扰线 """
            draw.line([random_dot(), random_dot()], fill=generate_fill_color())
        ...
        def create_points():
            ...
            [draw.point((w, h), fill=generate_fill_color()) for w in range(width) for h in range(height) if random.randint(0, 100) < chance]
